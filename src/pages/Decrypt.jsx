...
function Decrypt() {
  ...
  function caesarDecrypt(text, shift) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (char.match(/[a-zA-Z]/)) {
        ...
        let decrypted = String.fromCharCode(((char.charCodeAt(0) - base - shift + 26) % 26) + base)
        stepList.push(`${char} - ${shift} = ${decrypted}`)
        output += decrypted
      } else {
        output += char
  ...
  function vigenereDecrypt(text, keyword) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (char.match(/[a-zA-Z]/)) {
        ...
        let decrypted = String.fromCharCode(((char.charCodeAt(0) - base - shift + 26) % 26) + base)
        stepList.push(`${char} - ${keyword[ki % keyword.length]}(${shift}) = ${decrypted}`)
        output += decrypted
        ki++
      } else {
        output += char
  ...
  async function handleDecrypt() {
    ...
    if (cipher === 'caesar') {
      ...
      res = vigenereDecrypt(ciphertext, key)
    }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white px-8 py-12">
      <h1 className="text-4xl font-bold text-purple-400 mb-8"> Decrypt</h1>
